...
const GEMINI_KEY =
  (window.__VREMEA__ && window.__VREMEA__.GEMINI_API_KEY) || "";
...
function computeActive() {
  ...
  const anchor = scrollTop + viewport * 0.3;
...
function updateDateTime() {
  ...
  if (el)
    el.textContent = new Date().toLocaleDateString("ro-RO", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric",
...
const RO_DB = [
  ["București", "Bucharest,RO", 1],
  ["Cluj-Napoca", "Cluj-Napoca,RO", 1],
  ["Timișoara", "Timisoara,RO", 1],
  ["Iași", "Iasi,RO", 1],
  ["Constanța", "Constanta,RO", 1],
  ["Craiova", "Craiova,RO", 1],
  ["Brașov", "Brasov,RO", 1],
  ["Galați", "Galati,RO", 1],
  ["Ploiești", "Ploiesti,RO", 1],
  ["Oradea", "Oradea,RO", 1],
  ["Brăila", "Braila,RO", 2],
  ["Bacău", "Bacau,RO", 2],
  ["Arad", "Arad,RO", 2],
  ["Pitești", "Pitesti,RO", 2],
  ["Sibiu", "Sibiu,RO", 2],
  ["Târgu Mureș", "Targu Mures,RO", 2],
  ["Baia Mare", "Baia Mare,RO", 2],
  ["Buzău", "Buzau,RO", 2],
  ["Satu Mare", "Satu Mare,RO", 2],
  ["Botoșani", "Botosani,RO", 2],
  ["Râmnicu Vâlcea", "Ramnicu Valcea,RO", 2],
  ["Suceava", "Suceava,RO", 2],
  ["Târgoviște", "Targoviste,RO", 2],
  ["Focșani", "Focsani,RO", 2],
  ["Tulcea", "Tulcea,RO", 2],
  ["Deva", "Deva,RO", 2],
  ["Alba Iulia", "Alba Iulia,RO", 2],
  ["Bistrița", "Bistrita,RO", 2],
  ["Reșița", "Resita,RO", 2],
  ["Sfântu Gheorghe", "Sfantu Gheorghe,RO", 2],
  ["Piatra Neamț", "Piatra Neamt,RO", 2],
  ["Sinaia", "Sinaia,RO", 2],
  ["Mangalia", "Mangalia,RO", 2],
  ["Predeal", "Predeal,RO", 2],
  ["Giurgiu", "Giurgiu,RO", 2],
  ["Vaslui", "Vaslui,RO", 2],
];
...
function searchRO(q) {
  ...
  return RO_DB.filter(([n]) => normalizeStr(n).includes(nq))
    .sort(([nA, , tA], [nB, , tB]) => {
      const a = normalizeStr(nA),
        b = normalizeStr(nB);
      if (a === nq && b !== nq) return -1;
      if (b === nq && a !== nq) return 1;
...
async function fetchAndShow(q) {
  ...
  try {
    const res = await fetch(
      `${WX_BASE}/geo/1.0/direct?q=${encodeURIComponent(q)}&limit=20&appid=${WX_KEY}`,
    );
    ...
    const api = data
      ...
      .map((c) =>
        c.country === "RO" && RO_NAMES[c.name]
          ? { ...c, name: RO_NAMES[c.name] }
          : c,
      )
      .filter((c) => c.country !== "RO")
      .sort((a, b) => scoreCity(a, nq) - scoreCity(b, nq))
      .slice(0, 3);
    if (cityInput.value.trim() === q.trim())
      renderSugg([...local, ...api].slice(0, 7));
  } catch (e) {
    console.error(e);
  }
}
...
function showSuggestions(v) {
  if (!v) {
    if (recentSearches.length) {
      suggestDiv.innerHTML = recentSearches
        .map(
          (c) => `
...
function geoDisplayLine(g) {
  return [g.admin3, g.admin2, g.admin1, g.countryName]
    .filter(Boolean)
    .join(", ");
}
...
geoModal
  ?.querySelector(".geo-modal-backdrop")
  ?.addEventListener("click", closeGeoModal);
...
function openGeoModal(results, onPick) {
  ...
  geoModalList.innerHTML = results
    .map(
      (r, i) => `
...
async function smartSearch(rawQuery, interactive = true) {
  ...
  try {
    ...
    setLoading(false);
    if (interactive)
      alert("Nu am putut contacta serviciul de localizare. Încearcă din nou.");
    return;
  }
  setLoading(false);
...
function updateUI(d, geo) {
  ...
  if (geo) {
    ...
    const sub = [geo.admin2, geo.admin1, geo.countryName || geo.country]
      .filter(Boolean)
      .join(", ");
  ...
  set(
    "daylight-duration",
    `${Math.floor(dl / 3600000)}h ${Math.floor((dl % 3600000) / 60000)}m`,
  );

  const map = document.getElementById("google-map");
  if (map)
    map.src = `https://www.google.com/maps?q=${d.coord.lat},${d.coord.lon}&output=embed`;
...
function setExtra(d, geo) {
  ...
  set(
    "elevation",
    geo && Number.isFinite(geo.elevation)
      ? `${Math.round(geo.elevation)} m`
      : "—",
  );
}
...
async function fetch5Day(lat, lon) {
  try {
    const res = await fetch(
      `${WX_BASE}/data/2.5/forecast?lat=${lat}&lon=${lon}&units=metric&lang=ro&appid=${WX_KEY}`,
    );
    const data = await res.json();
...
async function fetchHourly(lat, lon) {
  try {
    ...
    if (!el) return;
    el.innerHTML = data.list
      .slice(0, 12)
      .map((h) => {
        const time = new Date(h.dt * 1000).toLocaleTimeString("ro-RO", {
          hour: "2-digit",
          minute: "2-digit",
        });
...
async function fetchAirQuality(lat, lon) {
  try {
    const res = await fetch(
      `${WX_BASE}/data/2.5/air_pollution?lat=${lat}&lon=${lon}&appid=${WX_KEY}`,
    );
    const data = await res.json();
    ...
    if (vEl) {
      vEl.textContent = aqi;
      vEl.className = "aqi-num " + classes[aqi - 1];
    }
    if (lEl) {
      lEl.textContent = labels[aqi - 1];
      lEl.className = "aqi-lbl " + classes[aqi - 1];
    }
    const set = (id, v) => {
      const el = document.getElementById(id);
      if (el) el.textContent = v;
...
const AFF = {
  booking: (c) =>
    `https://www.booking.com/searchresults.html?ss=${encodeURIComponent(c)}&aid=YOUR_AID`,
  gyg: (c) =>
    `https://www.getyourguide.com/s/?q=${encodeURIComponent(c)}&partner_id=YOUR_ID`,
  viator: (c) =>
    `https://www.viator.com/search/${encodeURIComponent(c)}?pid=YOUR_PID`,
  airbnb: (c) => `https://www.airbnb.com/s/${encodeURIComponent(c)}/homes`,
  tripadvisor: (c) =>
    `https://www.tripadvisor.com/Search?q=${encodeURIComponent(c)}`,
  facebook: (c, q) =>
    `https://www.facebook.com/search/groups/?q=${encodeURIComponent(q || c)}`,
};
  ...
  if (tier === "large") {
    buttons = [
      ...
      },
      {
        ...
        icon: "🎟",
        name: "GetYourGuide",
        sub: "Tururi & Activități",
      },
      {
        cls: "aff-viator",
        href: AFF.viator(city),
        icon: "🌍",
        name: "Viator",
        sub: "Experiențe locale",
      },
      {
        cls: "aff-airbnb",
        href: AFF.airbnb(city),
        icon: "🏡",
        name: "Airbnb",
        sub: "Cazări unice",
      },
    ...
    buttons = [
      ...
      {
        cls: "aff-facebook",
        href: AFF.facebook(city, fbQuery(city, country)),
        icon: "👥",
        name: "Grupuri Facebook",
        sub: "Grupuri locale",
      },
    ];
  } else {
...
function catLabel(kinds) {
  ...
  for (const key of Object.keys(CAT_MAP)) {
    if (k.includes(key)) return CAT_MAP[key];
  }
  return "Atracție";
}
...
function buildCardLinks(place, city, tier, country) {
  ...
  if (place.point) {
    ...
    links.push(
      `<a class="tc-link secondary" href="https://www.google.com/maps/search/${searchQ}/@${lat},${lon},15z" target="_blank" rel="noopener">🗺 Hartă</a>`,
    );
  } else {
    links.push(
      `<a class="tc-link secondary" href="https://www.google.com/maps/search/${searchQ}" target="_blank" rel="noopener">🗺 Hartă</a>`,
    );
  }
  ...
  if (wikiSlug && !isBadPlaceName(wikiSlug)) {
    ...
    links.push(
      `<a class="tc-link secondary" href="${wikiUrl}" target="_blank" rel="noopener">📖 Wikipedia</a>`,
    );
  }

  if (tier === "large") {
    links.push(
      `<a class="tc-link secondary" href="${AFF.gyg(name + " " + city)}" target="_blank" rel="noopener">🎟 Tur ghidat</a>`,
    );
  }

  if (tier !== "small") {
    links.push(
      `<a class="tc-link secondary" href="https://www.tripadvisor.com/Search?q=${searchQ}" target="_blank" rel="noopener">⭐ Recenzii</a>`,
    );
  }
...
function renderCards(places, city, tier, country) {
  ...
  const valid = places
    .filter((p) => {
      ...
      const desc =
        p.wikipedia_extracts?.text || p.info?.descr || p.description || "";
  ...
  el.innerHTML = valid
    .map((p) => {
      const desc =
        ...
        p.info?.descr ||
        p.description ||
        "Atracție locală recomandată.";
      const descClean = desc.length > 220 ? desc.slice(0, 220) + "…" : desc;
      const source = p.wikipedia_extracts?.text
        ? '<span class="tc-source">📡 Wikipedia</span>'
        : "";
      return `
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (
            ...
            p.desc =
              j.extract.length > 240
                ? j.extract.slice(0, 240) + "…"
                : j.extract;
            p.wikiUrl =
...
function renderTravelCards(places, geo) {
  ...
  el.innerHTML = places
    .map((p) => {
      ...
      const distLabel =
        distKm < 1 ? `${Math.round(p.distM)} m` : `${distKm.toFixed(1)} km`;
      const desc =
        p.desc ||
        `${p.cat} la ${distLabel} de centrul localității ${geo.name}.`;
      const links = [];
      links.push(
        `<a class="tc-link secondary" href="https://www.google.com/maps/search/?api=1&query=${p.lat},${p.lon}" target="_blank" rel="noopener">🗺 Hartă</a>`,
      );
      if (p.wikiUrl)
        links.push(
          `<a class="tc-link secondary" href="${p.wikiUrl}" target="_blank" rel="noopener">📖 Wikipedia</a>`,
        );
      if (p.website)
        links.push(
          `<a class="tc-link secondary" href="${p.website}" target="_blank" rel="noopener">🌐 Site oficial</a>`,
        );
      links.push(
        `<a class="tc-link secondary" href="https://www.tripadvisor.com/Search?q=${encodeURIComponent(p.name + " " + geo.name)}" target="_blank" rel="noopener">⭐ Recenzii</a>`,
      );
      return `
      <div class="travel-card">
...
async function initGlobe() {
  ...
  try {
    ...
    // Earth — equirectangular Blue Marble. Try a few CORS-enabled mirrors.
    ...
    let earthTex = null;
    for (const url of TEX_URLS) {
      earthTex = await new Promise((res) => {
    ...
    renderer.domElement.addEventListener(
      ...
      (e) => {
        ...
        camera.position.z = Math.max(
          1.4,
          Math.min(6, camera.position.z + e.deltaY * 0.0015),
        );
      },
      { passive: false },
    );
...
function airbnbLink(q) {
  return `https://www.airbnb.com/s/${encodeURIComponent(q)}/homes`;
}
function tripadvisorLink(q) {
  return `https://www.tripadvisor.com/Search?q=${encodeURIComponent(q)}`;
}
function skyscannerLink(q) {
  return `https://www.skyscanner.net/transport/flights-to/?search=${encodeURIComponent(q)}`;
}
...
async function tool_get_weather({ location }) {
  ...
  const g = matches[0];
  const r = await fetch(
    `${WX_BASE}/data/2.5/weather?lat=${g.lat}&lon=${g.lon}&units=metric&lang=ro&appid=${WX_KEY}`,
  );
...
const TOOL_DECLARATIONS = [
  {
    name: "get_weather",
    description:
      "Vremea curentă pentru un oraș/locație. Returnează temperatura, descrierea, vântul, umiditatea.",
    parameters: {
      type: "object",
      properties: {
        location: {
          type: "string",
          description:
            'Numele orașului sau localității, ex: "Cluj-Napoca, RO".',
        },
      },
      required: ["location"],
    },
  ...
  },
  {
    name: "compare_cities",
    description:
      'Compară vremea actuală în mai multe orașe simultan. Folosește când utilizatorul vrea „cel mai cald/rece/ploios" oraș dintr-o listă.',
    parameters: {
      type: "object",
...
async function runAI(userText) {
  if (!GEMINI_KEY) {
    addAIBubble(
      "bot",
      "Asistentul AI nu este configurat (lipsește cheia API).",
    );
    return;
  }
  ...
  try {
    ...
    while (safety++ < 5) {
      ...
      const recs = [];
      const cleaned = text
        .replace(/^RECOMMENDATION:\s*(.+)$/gim, (_, p) => {
          recs.push(p.trim());
          return "";
        })
        .trim();
    ...
    typing.querySelector(".ai-bubble").textContent =
      "Răspunsul a depășit limita de pași. Încearcă să reformulezi.";
  } catch (err) {
    console.error(err);
...
cityInput.addEventListener("focus", () => {
  if (!cityInput.value) showSuggestions("");
});
cityInput.addEventListener("input", (e) => showSuggestions(e.target.value));
cityInput.addEventListener("blur", () =>
  setTimeout(() => suggestDiv.classList.remove("active"), 200),
);
...
searchBtn.addEventListener("click", () => {
  if (cityInput.value.trim()) checkWeather(cityInput.value.trim());
});
...
locateBtn.addEventListener("click", () => {
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) =>
      checkByCoords(pos.coords.latitude, pos.coords.longitude).then(() => {
        locateBtn.innerHTML =
          '<i data-lucide="locate"></i><span>Locația mea</span>';
        safeIcons();
      }),
    () => {
